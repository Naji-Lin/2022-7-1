...
export default {
  ...
  methods: {
    ...
    async getPageList(pager = 1) {
      ...
      let result = await this.$API.ownerTrademark.reqTradeMarkList(page, limit);
      if (result.code == 200) {
        //分别是展示数据总条数与列表展示的数据
    ...
    addOrUpdateTradeMark() {
      ...
      this.$refs.ruleForm.validate(async (success) => {
        ...
        if (success) {
          ...
          //发请求（添加品牌|修改品牌）
          let result = await this.$API.ownerTrademark.reqAddOrUpdateTradeMark(this.tmForm);
          if (result.code == 200) {
            //弹出信息:添加品牌成功、修改品牌成功
            this.$message({
              type: "success",
    ...
    deleteTradeMark(row) {
      ...
      this.$confirm(`你确定删除${row.tmName}?`, "提示", {
        ...
        .then(async () => {
          //当用户点击确定按钮的时候会出发
          //向服务器发请求
          let result = await this.$API.ownerTrademark.reqDeleteTradeMark(row.id);
          //如果删除成功
          if (result.code == 200) {
            this.$message({
              type: "success",
