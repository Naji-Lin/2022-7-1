...
import * as ownerAttr from "@/api/product/ownerAttr";
...
export default {
  ...
  methods: {
    ...
    async getAttrList() {
      ...
      //获取属性列表的数据
      let result = await this.$API.ownerAttr.reqAttrList(
        category1Id,
        category2Id,
        category3Id
      );
    ...
    addAttrValue() {
      ...
      this.attrInfo.attrValueList.push({
        attrId: this.attrList.id, //对于修改某一个属性的时候，可以在已有的属性值基础之上新增新的属性值（新增属性值的时候，需要把已有的属性的id带上）
        valueName: "",
        flag: true,
      });
    ...
    async addOrUpdateAttr() {
      ...
      try {
        //发请求
        await this.$API.ownerAttr.reqAddOrUpdateAttr(this.attrInfo);
        //展示平台属性的信号量进行切换
