...
export default {
  ...
  methods: {
    ...
    async initSpuData(spu) {
      //获取SPU信息的数据
      let spuResult = await this.$API.ownerSpu.reqSpu(spu.id);
      if (spuResult.code == 200) {
        //在修改spu的时候,需要向服务器发请求的，把服务器返回的数据（对象），赋值给spu属性
        this.spu = spuResult.data[0];
      }
      //获取品牌的信息
      let tradeMarkResult = await this.$API.ownerSpu.reqTradeMarkList();
      if (tradeMarkResult.code == 200) {
        this.tradeMarkList = tradeMarkResult.data;
      }
      //获取spu图片的数据
      let spuImageResult = await this.$API.ownerSpu.reqSpuImageList(spu.id);
      ...
      //获取平台全部的销售属性
      let saleResult = await this.$API.ownerSpu.reqBaseSaleAttrList();
    ...
    async addOrUpdateSpu() {
      ...
      //发请求
      let result = await this.$API.ownerSpu.reqAddOrUpdateSpu(this.spu);
      if (result.code == 200) {
        //提示
