...
export default {
  ...
  methods: {
    ...
    async getData(category1Id, category2Id, spu) {
      ...
      this.spu = spu;
      //获取图片的数据
      let result = await this.$API.ownerSpu.reqSpuImageLIst(spu.id);
      if (result.code == 200) {
      ...
      //获取销售属性的数据
      let result1 = await this.$API.ownerSpu.reqSpuSaleAttrList(spu.id);
      if (result1.code == 200) {
        this.spuSaleAttrList = result1.data;
      }
      //获取平台属性的数据
      let result2 = await this.$API.ownerSpu.reqAttrInfoList(
        category1Id,
        category2Id,
        spu.category3Id
      );
    ...
    async save() {
      ...
      skuInfo.skuAttrValueList = attrInfoList.reduce((prev, item) => {
        if (item.attrIdAndValueId) {
          ...
          let valueName = item.attrValueList.forEach((value) => {
            if(value.attrId == attrId && value.valueId == valueId){
      ...
      //发请求
      let result = await this.$API.ownerSpu.reqAddSku(skuInfo);
      if (result.code == 200) {
