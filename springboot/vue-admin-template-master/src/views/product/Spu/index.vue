...
export default {
  ...
  methods: {
    ...
    async getSpuList(pages = 1) {
      ...
      let result = await this.$API.ownerSpu.reqSpuList(page, limit, category3Id);
      if (result.code == 200) {
    ...
    //删除SPU的回调
    async deleteSpu(row) {
      let result = await this.$API.ownerSpu.reqDeleteSpu(row.id);
      if (result.code == 200) {
    ...
    async handler(spu) {
      ...
      this.spu = spu;
      //获取sku列表的数据进行展示
      let result = await this.$API.ownerSpu.reqSkuList(spu.id);
      if (result.code == 200) {
